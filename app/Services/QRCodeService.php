...
class QRCodeService
{
    ...
    public function generateForMeeting(int $meetingId, string $tenantSlug): array
    {
        ...
        return [
            'code' => $uniqueCode,
            // Mismo motivo que en getQRCodeBase64(): HtmlString → string.
            'svg' => (string) $svgQR,
    ...
    public function getQRCodeBase64(string $qrCode): ?array
    {
        ...
        return [
            // `QrCode::generate()` devuelve un HtmlString: al serializar la
            // respuesta a JSON saldría como objeto y el cliente no podría
            // pintarlo. Se castea para que viaje como string (Spec 0021, F8).
            'svg' => (string) $svgQR,
            'svg_base64' => base64_encode($svgQR),
            'url' => $url,
        ];
